...
import type { AIConfig, ProviderConfig, TestConnectionResult } from '@ai-selector/core';
...
const inputClass = computed(() => {
  if (testStatus.value === 'success') return 'ai-input ai-input-success';
  if (testStatus.value === 'error') return 'ai-input ai-input-error';
  return 'ai-input ai-input-default';
});
...
<template>
  <div class="ai-card">
    ...
    <div class="space-y-3">
      ...
      <div class="space-y-2" ref="providerRef">
        ...
        <div class="relative">
          <button
            ...
            :disabled="disabled"
            class="ai-select-trigger"
          >
            <span v-if="provider" class="flex items-center gap-2 min-w-0">
              <img :src="provider.icon" :alt="provider.name" class="ai-provider-icon" />
              <span class="truncate">{{ provider.name }}</span>
            </span>
            <span v-else class="text-gray-500 dark:text-zinc-500">选择 Provider...</span>
            <svg class="w-4 h-4 opacity-50 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4" />
            </svg>
          </button>

          <div v-if="providerOpen" class="ai-dropdown max-h-[300px] overflow-auto p-1">
            <button
              v-for="p in providers"
              :key="p.id"
              type="button"
              @click="selectProvider(p.id)"
              :class="['ai-dropdown-item rounded-sm', providerId === p.id && 'ai-dropdown-item-active']"
            >
              <div class="flex items-center gap-2 flex-shrink-0">
                <img :src="p.icon" :alt="p.name" class="ai-provider-icon" />
                <span class="font-medium whitespace-nowrap">{{ p.name }}</span>
              </div>
              <span class="ai-hint-text ml-auto text-right truncate flex-1 min-w-0">
        ...
        >
          <div class="overflow-hidden">
            <input
              ...
              :disabled="disabled"
              class="ai-input ai-input-default"
            />
          </div>
        </div>
      </template>
          ...
          <div v-if="provider && provider.authType !== 'none'" class="space-y-2">
            ...
            <div class="relative">
              ...
              <span class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center">
                <svg v-if="testStatus === 'testing'" class="ai-icon-spin" viewBox="0 0 24 24" fill="none">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                </svg>
                <svg v-else-if="testStatus === 'success'" class="ai-icon-success" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                </svg>
                <svg v-else-if="testStatus === 'error'" class="ai-icon-error" fill="currentColor" viewBox="0 0 24 24">
            ...
            >
                <div class="overflow-hidden">
                    <div class="pt-1 text-xs space-y-0.5">
                        <p class="text-red-500 dark:text-red-400 font-medium">连通性测试失败，请检查 API Key 的有效性</p>
          ...
          <div class="space-y-2" ref="modelRef">
            ...
            <div class="relative">
              ...
              >
                ...
                <span v-else class="text-gray-500 dark:text-zinc-500">选择 Model...</span>
                <svg class="w-4 h-4 opacity-50 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4" />
                </svg>
              </button>

              <div v-if="modelOpen" class="ai-dropdown">
                ...
                <div class="max-h-60 overflow-auto">
                  <button
                    ...
                    class="ai-dropdown-item bg-blue-50 dark:bg-blue-900/20 border-b"
                  >
                    <span class="text-blue-500">使用自定义: {{ modelSearch }}</span>
                  </button>
                  <button
                    ...
                    :class="['ai-dropdown-item', model === m.id && 'ai-dropdown-item-active']"
                  >
                    <span>{{ m.name }}</span>
                  </button>
                  <div v-if="filteredModels.length === 0 && !modelSearch" class="p-4 text-center ai-hint-text">
                    暂无模型数据
                  </div>
                </div>
              </div>
            </div>
            <p v-if="!apiKey && provider?.authType !== 'none'" class="ai-hint-text">
              输入 API Key 后可获取完整模型列表
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="pt-2">
        <button
          ...
          :class="['ai-btn w-full', saveStatus === 'saved' ? 'ai-btn-success' : 'ai-btn-primary']"
        >
    ...
    <div v-if="showPreview && provider" class="mt-6 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg p-4 border border-gray-200/50 dark:border-zinc-800">
      <h3 class="text-xs font-medium text-gray-500 dark:text-zinc-500 mb-2 uppercase tracking-wider">配置预览</h3>
